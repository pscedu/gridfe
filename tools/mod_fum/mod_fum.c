...
#else
#include<err.h>
#define mf_err(x,y,z) errx("%s: Err %d  on line %d in %s",x,y,__FILE__,__LINE__)
#define mf_warn(x,y,z) warnx("%s: Err %d on line %d in %s",x,y,__FILE__,__LINE__)
#endif
...
/* Prototypes */
void mf_kinit(krb5_inst_ptr k5, kbr5_prefs_ptr kprefs);
...
void mf_kinit(krb5_inst_ptr k5, kbr5_prefs_ptr kprefs)
{
	...
	err = krb5_get_validated_creds(k5->context, &k5->credentials, k5->principal,
					k5->cache, kprefs->sname);
	if(err)
...
void mf_kinit_setup(krb5_inst_ptr k5, krb5_prefs_ptr kprefs)
{
	...
	*/
	err = krb5_sname_to_principal(k5->context,
		NULL, NULL, KRB5_NT_SRV_HOST, &k5->principal);
	if(err)
	...
	*/
	...
	err = krb5_unparse_name(k5->context, k5->principcal, &k5->name);
	if(err)
...
{
	if(k5->credentials)
		krb5_free_creds_contents(k5->context, k5->credentials);
	if(k5->name)
